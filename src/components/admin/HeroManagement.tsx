...
import { useHeroImages, useDeleteHeroImage, useUploadHeroImages } from '@/hooks/useHero';
...
export function HeroManagement() {
  ...
  const { data: images, isLoading } = useHeroImages(true);
  const uploadMutation = useUploadHeroImages();
  ...
  const onPickAndUpload = async (pickedFiles: FileList | null) => {
    ...
    try {
      ...
      toast({ title: 'Hero images updated', description: `${files.length} image(s) uploaded.` });
      if (fileInputRef.current) fileInputRef.current.value = '';
    } catch (e: unknown) {
      const msg = e instanceof Error ? e.message : 'Upload failed';
      toast({ title: 'Hero upload failed', description: msg, variant: 'destructive' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="text-left">
        <h1 className="text-2xl sm:text-3xl font-display font-bold mb-2 break-words">Hero Images</h1>
        <p className="text-sm sm:text-base text-muted-foreground break-words">
          Manage the homepage hero carousel images. Upload stores images directly to S3 (when enabled) and deletion removes them.
        </p>
      </div>

      <Card>
        <CardHeader className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <CardTitle className="text-lg">Upload new hero images</CardTitle>
          <Button
            variant="outline"
            size="sm"
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">Current hero images</CardTitle>
        ...
        <CardContent>
          {isLoading ? (
            ...
            <div className="text-center py-10 text-muted-foreground">
              No hero images uploaded yet. Your homepage will use the built-in fallback images.
            </div>
          ) : (
